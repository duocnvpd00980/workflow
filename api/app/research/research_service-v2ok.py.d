...
async def _scrape_facebook(fb_url: str, headless: bool, business_id: str):
    """
    HÀM CÀO DỮ LIỆU FACEBOOK - BẤT TỬ + CLICK EXPAND + SAVE POPUP DIALOG
    """
    browser = None
    ...
    try:
        ...
        # Loại bỏ sạch ký tự rác và dấu nháy để tạo câu lệnh site: chuẩn chỉ gửi lên Google
        ...
        # =====================================================
        # ĐỢI ĐÚNG 3 GIÂY CỐ ĐỊNH SAU KHI CLICK THEO YÊU CẦU
        # =====================================================
        ...
        # Bước 2: Nếu không đẻ tab mới, lấy luôn tab hiện tại (page)
        if not fb_tab:
        ...
        # Giữ nguyên số round hoặc tăng tùy ý (bản cũ của bạn chạy 2 round)
        ...
        print("📸 Lấy dữ liệu HTML tổng thể...")
        ...
        # =====================================================
        # 👑 BỔ SUNG: CLICK "SEE MORE" & "VIEW MORE COMMENTS"
        # =====================================================
        print("📜 Đang kích hoạt JS để mở rộng: See more / View more comments...")
        try:
            ...
            print(f"⚠️ Lỗi khi chạy script Expand: {e}")

        await asyncio.sleep(random.uniform(0.6, 1.2))
        print("✅ Hoàn tất kích hoạt expand.")

        # =====================================================
        # 👑 BỔ SUNG: KIỂM TRA & TRÍCH XUẤT DIALOG POPUP
        # =====================================================
        ...
        try:
            ...
            """)
            print(f"🔎 [POPUP DIALOG CHECK] Tìm thấy: {dialog_count} phần tử [role='dialog']")
            ...
            if popup_html != "NO_DIALOG":
                print(f"📦 Kích thước nội dung Popup: {len(popup_html)} ký tự.")
                with open(popup_path, "w", encoding="utf-8") as f:
                    f.write(popup_html)
                print(f"💾 Đã xuất file popup riêng biệt: {popup_path}")
            else:
                print("ℹ️ Giao diện hiện tại không xuất hiện Popup dialog.")
                popup_path = None

        except Exception as e:
            print(f"⚠️ Gặp lỗi trong quá trình xử lý Popup debug: {e}")
            popup_path = None
