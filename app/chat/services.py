...
def _make_thread_id(conversation_id: str, session_id: str) -> str:
    if conversation_id: return f"conv_{conversation_id}"
    return f"sess_{session_id}"

def _node_label(node_key: str) -> str:
    return NODE_LABELS.get(node_key) or NODE_LABELS.get(node_key.removeprefix("node_")) or node_key
...
def _get_or_create_conversation(user, conversation_id: str) -> Conversation:
    if user is not None:
        conv, _ = Conversation.objects.get_or_create(id=conversation_id, defaults={"user": user, "title": ""})
    else: conv = Conversation.objects.get(id=conversation_id)
    return conv

@sync_to_async(thread_sensitive=False)
def _create_user_message(conversation: Conversation, content: str, msg_id: str) -> Message:
    return Message.objects.create(id=msg_id, conversation=conversation, role=Message.Role.USER, status=Message.Status.COMPLETED, content=content)

@sync_to_async(thread_sensitive=False)
def _create_assistant_message_pending(conversation: Conversation, msg_id: str) -> Message:
    return Message.objects.create(id=msg_id, conversation=conversation, role=Message.Role.ASSISTANT, status=Message.Status.STREAMING, content="", html="")

@sync_to_async(thread_sensitive=False)
def _update_assistant_message(msg_id: str, payload: dict, status: str = Message.Status.COMPLETED, error_message: str = "") -> None:
    Message.objects.filter(id=msg_id).update(
        content=next((c.get("text", "") for c in payload.get("components", []) if c.get("type") == "text_response"), ""),
...
def _update_assistant_message_interrupted(msg_id: str, payload: dict) -> None:
    Message.objects.filter(id=msg_id).update(html=json.dumps(payload, ensure_ascii=False), status=Message.Status.INTERRUPTED, updated_at=timezone.now())

@sync_to_async(thread_sensitive=False)
def _touch_conversation(conversation_id: str) -> None:
    Conversation.objects.filter(id=conversation_id).update(last_message_at=timezone.now(), updated_at=timezone.now())
...
def _get_conversation_messages(conversation_id: str, limit: int = 20) -> list[dict]:
    messages = Message.objects.filter(conversation_id=conversation_id, status=Message.Status.COMPLETED).order_by("-created_at")[:limit]
    return list(reversed([{"id": str(m.id), "role": m.role, "status": m.status, "content": m.content, "payload": _safe_parse_json(m.html)} for m in messages]))
...
def _build_result(components: list[dict]) -> dict: return {"components": components}
def _build_text_response(text: str, title: str = "Kết quả") -> dict: return {"type": "text_response", "title": title, "text": text}
def _build_empty_state(message: str = "Đang xử lý...") -> dict: return {"type": "empty_state", "message": message}

def _build_error(error: Exception | None, code: str = "PIPELINE_CRASH") -> dict:
    if isinstance(error, TimeoutError): msg, code = "Hệ thống đang tải quá lâu. Vui lòng thử lại.", "TIMEOUT_ERROR"
    elif error is not None: msg = "Đã xảy ra lỗi trong quá trình xử lý. Vui lòng thử lại."
    else: msg, code = "Hệ thống chưa tạo được nội dung kết quả.", "EMPTY_STATE"
    debug = "".join(traceback.format_exception(type(error), error, error.__traceback__)) if error is not None else "Emergency Fallback."
    return {"type": "error", "message": msg, "code": code, "debug": debug}

def _build_human_review(draft: str, instruction: str, session_id: str = "", msg_id: str = "", conversation_id: str = "") -> dict:
    return {"type": "human_review", "draft": draft, "instruction": instruction, "session_id": session_id, "msg_id": msg_id, "conversation_id": conversation_id}
...
def _build_component(comp: dict) -> dict | None:
    ...
    if not props and comp.get("text"): props = {"text": comp["text"], "title": comp.get("title", "Kết quả")}
    return {"type": cid, **props}

def _snapshot_to_json(snapshot, error: Exception | None, session_id: str = "", msg_id: str = "", conversation_id: str = "") -> dict:
    if snapshot is None or error: return _build_result([_build_error(error)])
        ...
        if snapshot.tasks:
            for task in snapshot.tasks:
                if task.interrupts:
                    ...
                    if isinstance(val, dict): draft = val.get("draft") or val.get("text", "")
                    break
        if not draft:
            og_raw = state.get("output_guard")
            if isinstance(og_raw, dict): draft = og_raw.get("payload", {}).get("text", "")
        return _build_result([_build_human_review(draft=draft, instruction="Review nội dung. Chọn Duyệt hoặc Từ chối kèm phản hồi.", session_id=session_id, msg_id=msg_id, conversation_id=conversation_id)])
    ...
    if isinstance(final_node, dict):
        ...
        if components:
            ...
            if built: return _build_result(built)
        if text: return _build_result([_build_text_response(text)])
    
    hr_raw = state.get("human_review")
    if isinstance(hr_raw, dict):
        payload = hr_raw.get("payload", {})
        if payload.get("status") == "SUCCESS" and payload.get("text"): return _build_result([_build_text_response(payload["text"])])
    ...
    try:
        ...
        async for event_type, value in stream_guarded(main_app, graph_input, thread_id):
            ...
            # 2. HỖ TRỢ STREAM TOKEN THỜI GIAN THỰC (Nếu stream_guarded có trả về từ LLM)
                ...
                interrupt_payload = json.loads(value) if isinstance(value, str) else value
                draft = interrupt_payload.get("draft") or interrupt_payload.get("text", "")
                instruction = interrupt_payload.get("instruction", "Review nội dung.")
                result = _build_result([_build_human_review(draft=draft, instruction=instruction, session_id=session_id, msg_id=msg_id, conversation_id=conversation_id)])
                if assistant_msg_id:
                    await _update_assistant_message_interrupted(assistant_msg_id, result)
        ...
        text_response = next((c for c in components if c.get("type") == "text_response"), None)
        ...
        try:
            ...
            if assistant_msg_id:
                await _update_assistant_message(assistant_msg_id, error_result, status=Message.Status.ERROR, error_message=str(e))
...
class ChatService:
    async def stream_graph(self, message: str, session_id: str, msg_id: str = "", user=None, conversation_id: str = ""):
        ...
        if user:
            try:
                ...
                history = [{"role": m["role"], "content": m["content"]} for m in past if m["content"]]
            except Exception:
                logger.exception("[stream_graph] DB setup failed conv=%s", conversation_id)
                assistant_msg_id = ""; history = []

        thread_id = _make_thread_id(conversation_id, session_id)
        initial_input = {"user_input": message, "language": "vi", "budget_limit": 2.0, "conversation_id": conversation_id, "msg_id": msg_id, "chat_history": history[-10:]}
        main_app = await get_main_app()
        async for chunk in _run_stream(main_app, thread_id, initial_input, session_id, msg_id=msg_id, assistant_msg_id=assistant_msg_id, conversation_id=conversation_id):
            yield chunk

    async def resume_graph(self, session_id: str, action: str, feedback: str = "", msg_id: str = "", conversation_id: str = "", user=None):
        ...
        thread_id = _make_thread_id(conversation_id, session_id)
        cmd = Command(resume={"action": action.strip().lower(), "feedback": feedback.strip()})
        main_app = await get_main_app()
        async for chunk in _run_stream(main_app, thread_id, cmd, session_id, msg_id=msg_id, assistant_msg_id=assistant_msg_id, conversation_id=conversation_id):
            yield chunk

    async def restore_session(self, session_id: str, conversation_id: str = ""):
        if conversation_id: return await _get_conversation_messages(conversation_id)
        thread_id = _make_thread_id("", session_id)
        main_app  = await get_main_app()
        snapshot  = await _safe_get_snapshot(main_app, thread_id)
        if snapshot: return _snapshot_to_json(snapshot, None)
        return None