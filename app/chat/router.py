...
async def stream_message(
    ...
    if not message or not session_id:
        raise HTTPException(400, "Thiếu message hoặc session_id.")
    ...
    async def event_stream():
        ...
        try:
            async for chunk in svc.chat.stream_graph(
                message, session_id, msg_id=msg_id, conversation_id=conversation_id
            ):
                yield chunk
        except Exception:
...
async def resume_message(
    ...
    if not payload.session_id.strip() or not payload.action.strip():
        raise HTTPException(400, "Thiếu session_id hoặc action.")
    ...
    async def event_stream():
        ...
        try:
            ...
            logger.exception("[resume] crashed")
            yield f"event: error\ndata: {json.dumps({'message': 'Lỗi hệ thống'})}\n\n"
            yield "event: done\ndata: {}\n\n"
...
async def restore_chat(
    ...
    if not session_id and not conversation_id:
        raise HTTPException(400, "Thiếu session_id hoặc conversation_id.")
    ...
    return {"type": "html", "html": result or "",
            "msg_id": payload.msg_id or "", "session_id": session_id}
...
async def conversation_load(
    ...
    return {
        ...
        "messages": [{"id": str(m.id), "role": m.role, "content": m.content,
                      "html": m.html, "created_at": m.created_at}
