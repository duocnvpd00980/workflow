...
class ValidationIssue(BaseModel):
    ...
    scope: str = Field(
        ..., description="Phạm vi lỗi (e.g., 'grammar', 'policy', 'format')"
    )
    severity: str = Field(
        ..., description="Mức độ: 'critical' (phải sửa) hoặc 'warning' (nên sửa)"
    )
...
class ValidatorOutput(BaseModel):
    ...
    is_valid: bool = Field(
        default=True, description="Trạng thái tổng quát: True nếu đạt chuẩn"
    )
    ...
    needs_rework: bool = Field(
        default=False, description="Cần quay lại Node trước để sửa không"
    )
    target_node: Optional[str] = Field(
        None, description="Node cần quay lại để sửa (e.g., 'BLOG_WRITER')"
    )
