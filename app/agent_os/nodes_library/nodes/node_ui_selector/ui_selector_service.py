...
class UISelectorService:
    ...
    def select_components(self, finalizer_payload: BodyFrame) -> LLMSelectorResponse:
        ...
        empty_keywords = ["chưa tìm thấy", "không tìm thấy", "không có dữ liệu", "không phù hợp", "xin lỗi"]
        ...
        if has_ads and has_email and has_blog:
            selections.append(ComponentSelection(component_id="campaign_summary", props={}))
            selections.append(ComponentSelection(component_id="ads_card", props={}))
            selections.append(ComponentSelection(component_id="email_template", props={}))
        ...
        if not selections:
            if flow_type in ["ads_only", "marketing"] and text_content:
                ...
                selections.append(ComponentSelection(component_id="text_response", props={}))
    ...
    def resolve_ui(self, selector_res: LLMSelectorResponse, finalizer_payload: BodyFrame) -> UISelectorOutput:
        """
        ...
        raw_ads = next((r for r in records if isinstance(r, dict) and "headline" in r), {})
        raw_email = next((r for r in records if isinstance(r, dict) and "subject" in r), {})
        raw_blog = next((r for r in records if isinstance(r, dict) and "title" in r), {})
        ...
        for selection in selector_res.selections:
            ...
            if cid == "error_card":
                raw_props = {
                    "message": finalizer_payload.text or "Hệ thống trục trặc, vui lòng thử lại.",
                    ...
                    "debug_details": context_data.get("exception_trace") or finalizer_payload.error
                }
            elif cid == "empty_state":
                raw_props = {
                    "title": "Không tìm thấy nội dung kết quả",
                    "description": finalizer_payload.text or "Hệ thống đã chạy hoàn tất nhưng không trích xuất được dữ liệu phù hợp.",
                ...
                raw_props = raw_ads if raw_ads else {"headline": "Chiến dịch quảng cáo", "body": finalizer_payload.text or ""}
            elif cid == "email_template":
                raw_props = raw_email if raw_email else {"subject": "Thông tin gửi tới bạn", "body": finalizer_payload.text or ""}
            elif cid == "blog_preview":
                raw_props = raw_blog if raw_blog else {"title": "Bài viết mới cập nhật", "content": finalizer_payload.text or ""}
            elif cid == "campaign_summary":
                # Đếm tổng số lượng chữ tổng quan của toàn bộ chiến dịch dữ liệu phẳng
                ready_list = []
                if raw_ads: ready_list.append("ads_card")
                if raw_email: ready_list.append("email_template")
                if raw_blog: ready_list.append("blog_preview")
                
                raw_props = {
                    "status": "completed" if finalizer_payload.status == "SUCCESS" else "pending",
            ...
            try:
                ...
                if not schema_model:
                    raise ValueError(f"Missing schema model class mapping for component_id: {cid}")
                ...
                template_path = registry_config.get("template_path", f"widgets/{cid}.html")
                ...
                print(f"⚠️ [UI_SELECTOR_SERVICE] Schema Validation Failed for '{cid}': {str(val_err)}")
                fallback_used = True
                status = "fallback"
                ...
                rendered_list.append(
                    RenderedComponent(
                        ...
                        props={"text": finalizer_payload.text or "Hệ thống lỗi sinh cấu trúc giao diện hiển thị phức tạp.", "title": "Phản hồi dự phòng hệ thống"},
