...
class ClipperOutput(BaseModel):
    ...
    source_id: str = Field(
        ..., description="ID của nguồn dữ liệu gốc (ví dụ: bundle_id hoặc doc_id)"
    )
    clips: List[str] = Field(
        default_factory=list,
        description="Danh sách các đoạn nội dung đã được trích xuất",
    )
    summary: str = Field(..., description="Bản tóm tắt ngắn gọn nội dung cốt lõi")
    tags: List[str] = Field(
        default_factory=list, description="Các từ khóa hoặc chủ đề nổi bật của nội dung"
    )
