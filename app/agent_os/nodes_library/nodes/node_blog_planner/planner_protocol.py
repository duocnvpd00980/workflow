...
class BlogPlanOutput(BaseModel):
    ...
    target_keywords: List[str] = Field(default_factory=list, description="Danh sách từ khóa SEO chủ chốt")
    outline: List[str] = Field(..., description="Dàn ý chi tiết các mục (H2, H3)")
    estimated_word_count: int = Field(default=1000, description="Độ dài dự kiến của bài viết")
    research_required: bool = Field(default=False, description="Đánh dấu nếu cần tìm kiếm thêm dữ liệu thực tế")
