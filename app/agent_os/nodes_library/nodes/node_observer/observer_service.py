...
class ObserverService:
    ...
    async def summarize(self, state: Any) -> ObserverOutput:
        ...
        usage = {
            "input_tokens": ObservationMetric(metric_name="Input Tokens", value=1250, unit="tokens"),
            "latency": ObservationMetric(metric_name="Execution Time", value=4.5, unit="seconds")
        }

        # Kiểm tra động phần tử chạy liền trước (Ví dụ: Audit Logger)
        if hasattr(state, "node_reg_audit_logger") and state.node_reg_audit_logger is not None:
        ...
        if hasattr(state, "node_reg_policy_engine") and state.node_reg_policy_engine is not None:
            policy_frame = state.node_reg_policy_engine
            if policy_frame.payload.status != "SUCCESS" or policy_frame.payload.state.get("route_to") == "dlq":
