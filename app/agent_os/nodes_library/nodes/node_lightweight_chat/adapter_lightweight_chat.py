...
async def node_LIGHTWEIGHT_CHAT(
    ...
    if not hasattr(state, "reg_policy_engine") or state.reg_policy_engine is None:
        error_message = "[LIGHTWEIGHT_CHAT] Topology Violation: 'reg_policy_engine' missing!"
    ...
    result = await service_module.run(user_input=str(user_brief), context=intent_context)
