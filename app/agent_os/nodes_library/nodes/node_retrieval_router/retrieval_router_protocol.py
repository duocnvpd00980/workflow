...
class RetrievalRouterInput(BaseModel):
    ...
    intent_context: Optional[str] = Field(default="qa", description="Ngữ cảnh intent từ Policy Engine")

class RetrievalRouterOutput(BaseModel):
    rewritten_query: str = Field(..., description="Câu hỏi đã được tối ưu hóa/bóc tách từ khóa cho Vector DB")
    retrieval_needed: bool = Field(default=True, description="Đèn xanh kích hoạt tầng QAR cào dữ liệu")
