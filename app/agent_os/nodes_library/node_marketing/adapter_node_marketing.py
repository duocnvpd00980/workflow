...
from agent_os.nodes_library.node_marketing.node_marketing_protocol import (
    MarketingOutput,
)
from agent_os.nodes_library.node_marketing.node_marketing_service import (
    MarketingService,
)
...
async def node_marketing(state: MainBus, config: RunnableConfig = None) -> dict:
    """
    ======================================================================
    ...
    if not resolver_node or not hasattr(resolver_node, "payload"):
        error_message = (
            "[node_marketing] Topology Violation: Upstream Supervisor missing."
        )
    elif resolver_node.payload.status == "FAILED":
        error_message = (
            f"[node_marketing] Upstream Failure: {resolver_node.payload.error}"
        )
    ...
    try:
        ...
        domain_service = MarketingService(
            llm_engine=ctx.llm_factory.get_model("default")
        )
        domain_result = await domain_service.generate_marketing_content(
    ...
    return StandardFrame.emit(
        ...
        payload=BodyFrame(
            ...
            records=[],
            state={
                "audience": domain_result.target_audience if domain_result else None,
